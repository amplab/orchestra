...
def create_matrix(context, n):
    mat = np.random.rand(3, 3)
    return hermes.make_tensor(mat)

def create_dist_matrix(context, n):
    objrefs = np.zeros((2, 2), dtype="int64")
    for i in range(2):
        for j in range(2):
            objrefs[i,j] = context.call("create_matrix", 1)
...
context.register("create_matrix", hermes.Function(create_matrix, hermes.Int, hermes.Tensor))
context.register("create_dist_matrix", hermes.Function(create_dist_matrix, hermes.Int, hermes.Tensor))
